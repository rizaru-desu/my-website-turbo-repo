...
func (*Session) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case session.FieldID, session.FieldToken, session.FieldIPAddress, session.FieldUserAgent, session.FieldUserID, session.FieldImpersonatedBy:
	...
	for i := range columns {
		...
		case session.FieldImpersonatedBy:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				_m.ImpersonatedBy = new(string)
				*_m.ImpersonatedBy = value.String
			}
		default:
